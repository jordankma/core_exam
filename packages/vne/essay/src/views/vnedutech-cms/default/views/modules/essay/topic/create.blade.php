...
{{-- Page title --}}
@section('title'){{ $title = trans('vne-essay::language.titles.topic.create') }}@stop
    ...
    <section class="content paddingleft_right15">
        ...
        <div class="row">
            <div class="the-box no-border">
                <!-- errors -->
                <form action="{{ route('vne.essay.topic.add') }}" method="post">
                    <input type="hidden" name="_token" value="{{ csrf_token() }}"/>
                    <div class="row">
                        <div class="col-sm-8">
                            <label> {{ trans('vne-essay::language.label.topic.name') }} </label>
                            <div class="form-group {{ $errors->first('name', 'has-error') }}">
                                <input type="text" name="name" class="form-control" placeholder="{{ trans('vne-essay::language.placeholder.topic.name_here') }}">
