...
{{-- Page title --}}
@section('title'){{ $title = trans('contest-contestmanage::language.titles.topic_round.create') }}@stop
    ...
    <section class="content paddingleft_right15">
        ...
        <div class="row">
            <div class="panel panel-primary" id="hidepanel1">
                ...
                <!-- errors -->
                {!! Form::open(array('url' => route('contest.contestmanage.topic_round.add'), 'method' => 'post', 'class' => 'bf', 'id' => 'examForm', 'files'=> true)) !!}
                <input type="hidden" name="_token" value="{{ csrf_token() }}"/>
                    <div class="row">
                        <div class="col-md-6">
                            <label>Tên vòng trong màn thi</label>
                            <div class="form-group {{ $errors->first('name', 'has-error') }}">
                                {!! Form::text('name', null, array('class' => 'form-control', 'autofocus'=>'autofocus','placeholder'=> trans('contest-contestmanage::language.placeholder.topic_round.name_here'))) !!}
                                <span class="help-block">{{ $errors->first('name', ':message') }}</span>
                            </div>
                            <label>Màn thi</label>
                            <div class="form-group {{ $errors->first('topic', 'has-error') }}">
                                {!! Form::text('topic', null, array('class' => 'form-control topic', 'readOnly' => 'readOnly', 'autofocus'=>'autofocus','placeholder'=> trans('contest-contestmanage::language.placeholder.topic_round.topic'))) !!}
                                <span class="help-block">{{ $errors->first('topic', ':message') }}</span>
                            </div>
                            {!! Form::hidden('topic_id', null, array('class' => 'form-control topic_id')) !!}
                            ...
                            {{--<div class="form-group {{ $errors->first('end_notify', 'has-error') }}">--}}
                                {{--{!! Form::textarea('end_notify', null, array('class' => 'form-control', 'autofocus'=>'autofocus','placeholder'=> trans('contest-contestmanage::language.placeholder.topic_round.after_end_notify'))) !!}--}}
                                {{--<span class="help-block">{{ $errors->first('end_notify', ':message') }}</span>--}}
                            {{--</div>--}}
                            <label>Mô tả</label>
                            <div class="form-group {{ $errors->first('description', 'has-error') }}">
                                {!! Form::textarea('description', null, array('class' => 'form-control', 'autofocus'=>'autofocus','placeholder'=> trans('contest-contestmanage::language.placeholder.topic_round.description'))) !!}
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="form-group {{ $errors->first('total_question', 'has-error') }}">
                                {!! Form::number('total_question', null, array('class' => 'form-control')) !!}
                                <span class="help-block">{{ $errors->first('total_question', ':message') }}</span>
                            </div>
                            <label>Tổng điểm tối đa</label>
                            <div class="form-group {{ $errors->first('total_point', 'has-error') }}">
                                {!! Form::number('total_point', null, array('class' => 'form-control')) !!}
                                <span class="help-block">{{ $errors->first('total_point', ':message') }}</span>
                            </div>
                            <label>Tổng thời gian giới hạn của vòng thi</label>
                            ...
                            <label>Trừ điểm nếu không trả lời</label>
                            <div class="form-group">
                                <input type="checkbox" name="point_minus_no_answer" class="allow_permission" data-size="mini" checked>
                            </div>
                            <label>Hiển thị câu trả lời đúng</label>
                            <div class="form-group">
                                <input type="checkbox" name="show_true_answer" class="allow_permission" data-size="mini" checked>
                            </div>
                            <div class="panel panel-primary">
                                <div class="panel-body">
                                    <p class="text-on-pannel text-primary"><strong> Ngôi sao may mắn </strong></p>
                                    <label>Trạng thái</label>
                                    <div class="form-group">
                                        <input type="checkbox" name="luckystar[status]" class="allow_permission" data-size="mini" checked>
                                    </div>
                                    <label>Trừ điểm nếu trả lời sai</label>
                                    <div class="form-group">
                                        <input type="checkbox" name="luckystar[point_minus]" class="allow_permission" data-size="mini" checked>
                                    </div>
                                    <label>Số lượng</label>
                                    <div class="form-group {{ $errors->first('luckystar[quantity]', 'has-error') }}">
                                        {!! Form::number('luckystar[quantity]', null, array('class' => 'form-control')) !!}
                                        <span class="help-block">{{ $errors->first('luckystar[quantity]', ':message') }}</span>
                                    </div>
                                    <label>Hệ số điểm</label>
                                    <div class="form-group {{ $errors->first('luckystar[point]', 'has-error') }}">
                                        {!! Form::number('luckystar[point]', null, array('class' => 'form-control')) !!}
                                        <span class="help-block">{{ $errors->first('luckystar[point]', ':message') }}</span>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="row">
                        ...
                        <div class="form-group {{ $errors->first('rules', 'has-error') }}">
                            {!! Form::textarea('rules', null, array('class' => 'form-control', 'id' => 'ckeditor_full','placeholder'=> trans('contest-contestmanage::language.placeholder.topic_round.rules'))) !!}
                    ...
                    <div class="row">
                        ...
                        <div class="form-group">
                            ...
                            <a href="{!! route('contest.contestmanage.topic_round.create') !!}"
        ...
        <div class="modal fade in" id="topic_list" tabindex="-1" role="dialog" aria-hidden="false">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title mr-auto">Chọn màn thi</h4>
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                    </div>
                    <div class="modal-body">
                    </div>
...
@section('footer_scripts')
    ...
    <script>
        ...
        $('body').on('click','.topic', function () {
            ...
            $.post(href, {type: 'topic'}, function (res) {
                if(res) {
                    $('#topic_list .modal-body').html();
                    $('#topic_list .modal-body').html(res);
                    $('#topic_list').modal();
                }
            });
        });
        $('body').on('click','.topic_choose', function () {
            var topic_id = $(this).attr('data-value');
            var topic_name = $(this).attr('c-data');
            $('.topic').val(topic_name);
            $('.topic_id').val(topic_id);
            $('#topic_list').modal('hide');
        });
    </script>
@stop
